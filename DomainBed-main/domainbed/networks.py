...
class SepResBlock(nn.Module):
    def __init__(self, channels):
        ...
        # Cụm Sep 1
        self.sep1_dw = nn.Conv2d(channels, channels, kernel_size=(3, 1), groups=channels, padding=(1, 0), bias=False)
        self.sep1_pw = nn.Conv2d(channels, channels, kernel_size=(1, 1))
        self.act1 = HardSwish()
        
        # Cụm Sep 2
        self.sep2_dw = nn.Conv2d(channels, channels, kernel_size=(3, 1), groups=channels, padding=(1, 0), bias=False)
        self.sep2_pw = nn.Conv2d(channels, channels, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        # L.Add([x, h]) -> Residual Connection
        return x + h

class YHXCNet(nn.Module):
    def __init__(self):
        super(YHXCNet, self).__init__()
        
        # Stem: Input (None, 2, 18, 32) -> Giả định input channels là 18 dựa trên ảnh
        # stem_dw: groups=18, kernel=(3,1)
        self.stem_dw = nn.Conv2d(18, 18, kernel_size=(3, 1), groups=18, padding=(1, 0), bias=False)
        self.stem_pw = nn.Conv2d(18, 32, kernel_size=(1, 1))
        
        # Block 1
        self.blk1 = SepResBlock(32)
        
        # Head
        self.head_act = HardSwish()
        self.head_dw = nn.Conv2d(32, 32, kernel_size=(3, 1), groups=32, padding=(1, 0), bias=False)
        self.head_pw = nn.Conv2d(32, 8, kernel_size=(1, 1))
